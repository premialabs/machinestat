...
use App\Src\Site\gen\SiteFieldValidator;
...
class SiteBaseRepo extends BaseRepo
{
  ...
  public function createRec(array $rec)
  {
    ...
    $validator = Validator::make(
      $rec,
      SiteFieldValidator::getCreateRules()
    );
    if ($validator->fails()) {
    ...
    $object =  Site::create($rec);
    if ($this->modelAuditable('Site')) {
  ...
  {
    $object = Site::findOrFail($model_id);
    ...
    $validator = Validator::make(
      $rec,
      SiteFieldValidator::getUpdateRules($model_id)
    );
    if ($validator->fails()) {
      throw new Exception($validator->errors());
    }
    ...
    $_is_auditable = $this->modelAuditable('Site');
    if ($_is_auditable) {
...
public function deleteRec($model_id)
  ...
  public function showRec($model_id)
  {
    ...
    return SiteWithParentsView::collection($data);
  }
