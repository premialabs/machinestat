...
use App\Src\Reading\gen\ReadingFieldValidator;
...
class ReadingBaseRepo extends BaseRepo
{
  ...
  public function createRec(array $rec)
  {
    ...
    $validator = Validator::make(
      $rec,
      ReadingFieldValidator::getCreateRules()
    );
    if ($validator->fails()) {
    ...
    $object =  Reading::create($rec);
    if ($this->modelAuditable('Reading')) {
  ...
  {
    $object = Reading::findOrFail($model_id);
    ...
    $validator = Validator::make(
      $rec,
      ReadingFieldValidator::getUpdateRules($model_id)
    );
    if ($validator->fails()) {
      throw new Exception($validator->errors());
    }
    ...
    $_is_auditable = $this->modelAuditable('Reading');
    if ($_is_auditable) {
...
public function deleteRec($model_id)
  ...
  public function showRec($model_id)
  {
    ...
    $data = Reading::where('id', $model_id)
      ->orderBy('_seq')
      ->with([])
      ->get()
      ->map(
    ...
    return ReadingWithParentsView::collection($data);
  }
