...
use App\Src\Event\gen\EventFieldValidator;
...
class EventBaseRepo extends BaseRepo
{
  ...
  public function createRec(array $rec)
  {
    ...
    $validator = Validator::make(
      $rec,
      EventFieldValidator::getCreateRules()
    );
    if ($validator->fails()) {
    ...
    $object =  Event::create($rec);
    if ($this->modelAuditable('Event')) {
  ...
  {
    $object = Event::findOrFail($model_id);
    ...
    $validator = Validator::make(
      $rec,
      EventFieldValidator::getUpdateRules($model_id)
    );
    if ($validator->fails()) {
      throw new Exception($validator->errors());
    }
    ...
    $_is_auditable = $this->modelAuditable('Event');
    if ($_is_auditable) {
...
public function deleteRec($model_id)
  ...
  public function showRec($model_id)
  {
    ...
    return EventWithParentsView::collection($data);
  }
